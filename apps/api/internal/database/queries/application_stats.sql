...
-- name: GetSubmittedApplicationGenders :one
SELECT
...
-- name: GetSubmittedApplicationRaces :many
SELECT
    CASE 
...
-- name: GetSubmittedApplicationSchools :many
SELECT
...
-- name: GetSubmittedApplicationMajors :many
SELECT
    trim(major) AS major,
    COUNT(*) AS count
FROM applications,
...
-- name: GetApplicationStatuses :one
SELECT
