...
func getUserContext(w http.ResponseWriter, r *http.Request) (*middleware.UserContext, error) {
	userContext, ok := r.Context().Value(middleware.UserContextKey).(*middleware.UserContext)

	if !ok || userContext == nil {
		err := errors.New("user_error")

		log.Err(err).Msg("unable to fetch user info")

		res.SendError(w, http.StatusBadRequest, res.NewError(err.Error(), "Unable to fetch user information."))
		return nil, err
	}
...
func (h *EventHandler) GetApplicationByUserAndEventID(w http.ResponseWriter, r *http.Request) {
	...
	userContext, err := getUserContext(w, r)
	...
	application, err := h.eventService.GetApplicationByUserAndEventID(r.Context(), params)
	...
	if err != nil {
		if err == repository.ErrApplicationNotFound {
			...
			newApplication, err := h.eventService.CreateApplication(r.Context(), params)
			if err != nil {
...
func (h *EventHandler) SubmitApplication(w http.ResponseWriter, r *http.Request) {
	...
	// 	// Example: save to local disk (you might upload to S3/GCS instead)
	// 	dst, err := os.Create("./uploads/" + header.Filename)
	// 	if err != nil {
	// 		http.Error(w, "Failed to save resume: "+err.Error(), http.StatusInternalServerError)
	// 		return
	// 	}
	// 	defer dst.Close()

	// 	if _, err := io.Copy(dst, file); err != nil {
	// 		http.Error(w, "Failed to copy resume: "+err.Error(), http.StatusInternalServerError)
	// 		return
	// 	}
	// } else if err != http.ErrMissingFile {
	// 	http.Error(w, "Error reading resume: "+err.Error(), http.StatusInternalServerError)
	// 	return
	...
	userId := userContext.UserID
	h.eventService.SubmitApplication(r.Context(), submission, userId, eventId)

	w.WriteHeader(http.StatusOK)
}

func (h *EventHandler) SaveApplication(w http.ResponseWriter, r *http.Request) {
	var data any
	...
	userId := userContext.UserID
	h.eventService.SaveApplication(r.Context(), data, sqlc.UpdateApplicationParams{
		UserID:  userId,
		EventID: eventId,
	})
