...
CREATE TABLE campaigns (
    ...
    emails[] REFERENCES campaign_emails(id),
    -- statistics,
...
CREATE TABLE campaign_emails {
    ...
    header TEXT, -- TODO: check smpt spec for max char count
    body TEXT, -- TODO: check smpt spec for max char count
    template REFERENCES campaign_email_template(id),
...
CREATE TABLE campaign_email_template {
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    title TEXT CONSTRAINT title_charlim CHECK (char_length(title) <= 200),
    html TEXT, -- TODO: check smpt spec for max char count
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
}
...
CREATE TABLE campaign_email_statistics {
    ...
    link[] NOT NULL REFERENCES campaign_link(id),
