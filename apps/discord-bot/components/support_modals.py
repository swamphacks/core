...
class VCSupportModal(Modal, title="VC Support Inquiry"):
    ...
    This modal provides a form interface for users to submit support requests.
        It creates a private vc in the --- SwampHacks XII (Support-VCs) --- category and notifies staff members
        through the reports channel.
    """
    def __init__(self) -> None:
        """
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        This method:
        1. Validates the existence of required channels and roles
        2. Creates a private voice channel in the --- SwampHacks XII (Support-VCs) --- category for the user and mentors to discuss the issue
        ...
        if not reports_channel:
            ...
            return
        if category is None:
            await interaction.response.send_message("Category **--- SwampHacks XII (Support-VCs) ---** not found.", ephemeral=True)
            return
