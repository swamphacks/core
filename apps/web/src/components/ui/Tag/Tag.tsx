...
const colors = {
  gray: "bg-gray-100 text-gray-600 border-gray-200 hover:border-gray-300 dark:bg-zinc-700 dark:text-zinc-200 dark:border-zinc-600 dark:hover:border-zinc-500",
  green:
...
export const removeButtonStyles = tv({
  base: "m-0 cursor-default rounded-full transition-[background-color] p-0.5 flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10 pressed:bg-black/20 dark:pressed:bg-white/20",
});
        ...
        <>
          ...
          {allowsRemoving && (
            <Button slot="remove" className={removeButtonStyles}>
              <TablerX aria-hidden className="w-3 h-3" />
            </Button>
          )}
        </>
      )}
